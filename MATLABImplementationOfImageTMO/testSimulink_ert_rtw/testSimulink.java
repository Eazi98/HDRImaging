package com.example.testSimulink;
...
import com.example.testSimulink.databinding.ActivityMainBinding;
...
public class testSimulink extends AppCompatActivity implements CvCameraViewListener2, OnFragmentInteractionListener {
 ...
 // properties specific for Camera block
   ...
   private boolean isCamera2Supported() {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        com.example.testSimulink.databinding.ActivityMainBinding activityMainBinding = ActivityMainBinding.inflate(LayoutInflater.from(this));
...
hasCamera2Support = isCamera2Supported();
    ...
    private testSimulink thisClass;
    private final Thread BgThread = new Thread() {
    @Override
    public void run() {
            String argv[] = new String[] {"MainActivity","testSimulink"};
           ...
           case "dot1":
              if (hasCamera2Support) {
               ...
               if (checkIfAllPermissionsGranted()) {
                   resumeCamera();
	                System.loadLibrary("testSimulink");
                   if (!BgThread.isAlive())
                       BgThread.start();
               }
               break;
    ...
    private native int naMain(String[] argv, testSimulink pThis);
